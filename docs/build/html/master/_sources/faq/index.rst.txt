...
由于openEuler Embedded一般面对的场景是没有显示输出，没有输入设备，且软件的成分运行过程中基本不会发生变化，例如路由器、工业控制器、机器人控制器等，所以默认生成的镜像
...
再把根文件系统与其他部件打包成可以烧录或者安装的系统镜像。软件包管理器也是个软件包，因此也可以集成到镜像中，从而用户可以通过dnf或apt命令来安装软件包。但需要注意的是，
...
1. 查询openeuler Embedded的软件包manifest文件，确认所需要的软件包是否已集成到openEuler Embedded中：
