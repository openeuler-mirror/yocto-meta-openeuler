...
24.03-LTS版本之后，软总线从3.1版本升级到了3.2版本，支持5.10内核，暂不支持6.6内核。原有接口保持兼容，启动方式有所不同，3.2版本不再通过softbus_server_main启动软总线服务，而是通过脚本进行启动。各个子模块以组件化的方式进行协同，而不是原先的单独进程。
...
服务端的服务启动脚本为start_services.sh。

如部署模型，软总线通过独立进程部署的方式对外提供服务，通过执行服务端主程序可拉起软总线进程提供对外服务。

.. code-block:: console

  openeuler ~ # /system/bin/start_services.sh softbus
...
.. attention::
    ...
    * 用户请参考部署模型，保证单设备节点上有且仅有唯一的服务进程。

    * 用户需要在启动前配置/etc/SN，并保证多设备下SN号的唯一性
...
1. 编写客户端程序
	...
	${CC} -lsoftbus_client.z -lboundscheck -I=/usr/include/dsoftbus/ softbus_client_main.c -o softbus_client_main
...
3. 启动分布式软总线
   ...
   在qemu1和qemu2中启动分布式软总线的服务端
    ...
    .. code-block:: console
        ...
        /system/bin/start_services.sh softbus
...
.. code-block:: console
    ...
    #: ${CROSS_COMPILE}ld -ldeviceauth_sdk.z -lcjson -lbegetutil.z -I=/usr/include/device_auth/ -I=/usr/include/init/
...
参考:
 ...
 `树莓派SD卡烧录指导 <https://atomgit.com/openeuler/raspberrypi/blob/master/documents/%E5%88%B7%E5%86%99%E9%95%9C%E5%83%8F.md#%E5%88%B7%E5%86%99-sd-%E5%8D%A1>`_
...
3.2版本的软总线与3.1版本相比，主要的一些接口变化如下：

+----------------+-------------------------+-------------------------+
| 接口功能       | 3.1版本                 | 3.2版本                 |
+================+=========================+=========================+
| 发布服务       | PublishService          | PublishLNN              |
+----------------+-------------------------+-------------------------+
| 注销服务       | UnPublishService        | StopPublishLNN          |
+----------------+-------------------------+-------------------------+
| 发现服务       | StartDiscovery          | RefreshLNN              |
+----------------+-------------------------+-------------------------+
| 停止服务       | StopDiscovery           | StopRefreshLNN          |
+----------------+-------------------------+-------------------------+
...
服务端使用
----------------------
3.2版本的软总线启动方式与3.1版本有所不同。由 :file:`/system/bin/start_services.sh` 脚本进行启动软总线相关服务，而 :file:`/system/bin/stop_services.sh`  脚本关闭服务。如下：
...
如果以上没有解决你的问题，可以记录下分布式软总线的服务端日志和客户端日志，在 `分布式软总官方仓库 <https://atomgit.com/openeuler/dsoftbus_standard>`_ 提相关的issue，请尽量详细描述清楚你的操作步骤，包括自己所做的一些尝试。
