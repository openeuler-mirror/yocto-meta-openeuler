...
.. note:: 
    ...
    原来该目录下存在原生的meta-raspberrypi层，但是由于openEuler Embedded代码结构的管理，该层被移到openeuler/meta-raspberrypi，该仓库的链接地址为：https://atomgit.com/openeuler/yocto-meta-raspberrypi.git，在构建树莓派镜像时，我们需要树莓派的platform配置文件，该文件就是raspberrypi6-64.yaml，在该文件中引入了树莓派层，同时添加了repo下载信息，在repo中添加yocto-meta-raspberrypi是因为oebuild会在启用bitbake编译环境前下载该仓，而我们需要bitbake在解析菜谱前将所有需要的layer准备好。
