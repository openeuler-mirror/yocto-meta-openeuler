...
* git clone yocto-meta-openeuler仓库
    ...
    .. code-block:: bash
        ...
        git clone https://atomgit.com/openeuler/yocto-meta-openeuler.git
...
*  编译文档
   ...
   在 :file:`docs` 目录下编译文档：
    ...
    编译成功后，可以直接打开 :file:`docs/build/html/*.html` 查看最终生成的网页形式文档。

* 提交修改

  和提交代码类似，将所有修改通过commit的形式提交，然后在AtomGit上创建PR提交到openEuler Embedded对应的仓库，经过审查后，
  修改就会被CI自动编译并发布。
...
像提交代码一样，将所有修改通过commit的形式提交，然后在AtomGit上创建PR提交到openEuler Embedded对应的仓库, 经过审查后，修改就会被CI自动编译并发布。
