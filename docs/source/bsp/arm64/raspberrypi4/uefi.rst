...
********************************************
   ...
   openEuler Embedded 的树莓派镜像可在构建时通过 ``RPI_USE_UEFI`` 使能UEFI+GRUB引导，其混合关键部署镜像(openeuler-image-mcs for RPI4)默认使能了UEFI+GRUB，并集成了基于树莓派4B的混合部署环境依赖。

   凡构建时使能UEFI的openEuler Embedded树莓派镜像（不限于mcs镜像），后续的启动流程与注意事项均同样适用，以下基于mcs镜像介绍如何使用树莓派UEFI。
...
.. note::
   ...
   凡在构建时使能UEFI的openEuler Embedded树莓派镜像（openeuler-image-mcs默认使能），其boot分区集成的UEFI固件（基于 pftf v1.33 定制）在构建时均已预置UEFI配置：System Table Selection 为 DEVICETREE（关闭 ACPI），3G 内存限制已关闭。**此类镜像烧录后首次启动无需进入 UEFI 菜单配置**，可直接执行步骤 2 选择 SD 卡启动；步骤 1 的菜单操作仅在使用出厂原版 pftf 固件（见下文“手动刷新固件使树莓派4B支持UEFI引导”章节）或需要自定义 UEFI 设置时才需要执行。预置配置与 v1.33 固件版本绑定，后续升级固件版本后以实际行为为准。
...
2. **更改boot order或手动选择SD卡启动，如下，最后选择Commit Changes and Exit**
   ...
   **启动截图**
   ...
   .. attention::
      ...
      * 出厂原版 pftf 固件默认使用3G内存limit，可在UEFI菜单中关闭3G limit，否则系统启动后能看到的内存只有3G【`官方说明 <https://github.com/pftf/RPi4/>`_ 】；使能UEFI构建的镜像集成的固件已预置关闭3G limit，无需进入UEFI菜单设置

      * 出厂原版固件的UEFI+ACPI部署方式有缺陷（HDMI驱动异常），首次使用需进入UEFI菜单切换为DEVICETREE模式；使能UEFI构建的镜像集成的固件已预置为DEVICETREE模式
...
2. **下载树莓派UEFI固件**
   ...
   .. attention::
      ...
      * 3G内存解锁和关闭ACPI使能DEVICETREE请参考上述"openeuler-image-mcs启动使用指导"章节
...
**内核单独编译例子（openEuler）**
   ...
   .. attention::
      ...
      * 若使用上述UEFI+ACPI部署方法，必须在config中开启ACPI系列功能支持。在make menuconfig ARCH=arm64菜单中，选中ACPI默认系列支持。建议在UEFI中关闭ACPI选用DEVICETREE（参考openeuler-image-mcs启动使用指导相关配置)
