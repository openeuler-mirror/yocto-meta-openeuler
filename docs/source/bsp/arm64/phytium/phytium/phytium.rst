...
飞腾开发板统一镜像实现了硬件通用化适配，单一份镜像文件可兼容多款飞腾系列开发板，无需针对不同板型单独编译、定制镜像

双启动模式支持:
统一镜像兼容UEFI与UBOOT两种主流启动方式，可适配不同开发板的硬件启动架构

镜像格式:
可直接生成预分区镜像文件，支持两种主流镜像格式，分别为wic格式与genimage格式。镜像已提前完成分区规划，无需用户手动分区、调整参数，规避了手分区出错、参数适配不当等问题。

多存储介质兼容:
支持SATA硬盘、U盘、SD卡三类常用存储介质
...
构建完成后，输出文件见 /home/user/buildwork/phytium/output/[时间戳]，备用组件内容如下
...
若需要交叉编译工具链，可通过如下命令生成，输出文件位于output目录下带时间戳的子目录中。
...
烧录镜像的SATA硬盘/U盘安装盘插入主机，显示器通过VGA/HDMI连接板卡，等待系统启动后即可登录。
