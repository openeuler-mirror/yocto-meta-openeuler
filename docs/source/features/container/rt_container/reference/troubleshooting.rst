...
本文档提供 MicRun 常见问题的排查步骤和解决方案。排查时优先检查 ``runtime.json`` 快照；``state.json`` 为兼容格式的状态文件，在 ``runtime.json`` 缺失时可作回退排查入口。
...
.. list-table::
   ...
   * - Sandbox 状态不一致
     - 优先核对 ``/run/micrun/runtime/sandbox/<id>/runtime.json``，确认 stale 后再清理对应运行时目录
