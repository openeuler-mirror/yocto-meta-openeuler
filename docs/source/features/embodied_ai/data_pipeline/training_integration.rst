...
有关通过遥操作收集演示数据的信息，请参阅 `遥操作与数据收集 <#9.1>`__。有关将 ROS2 bag 转换为训练所需的 LeRobot 数据集格式，请参阅 `数据集转换 (bag_to_lerobot) <#9.3>`__。有关将训练后的模型部署回机器人，请参阅页面 `7.4 <#7.4>`__（策略节点）。
...
**ACT (Action Chunking Transformer)**：- 需要 ``chunk_size`` =
``n_action_steps``（通常为 100）- 最适合平滑、连续的操作任务 -
训练时间：单 GPU 上 50k 步约 12-24 小时
