...
该文档讲解如何安装并使能oebuild，该版本匹配oebuild版本为v0.0.30。

如何安装
========

oebuild是用python语言编写，并且适配的python版本为python3。目前oebuild已经在PyPi平台发布，因此可以通过pip3命令进行安装或升级oebuild。由于openEuler Embedded的构建目前仅适配常用的x86架构，因此对于其他架构的底层硬件并不适用，同时对于x86架构的上层系统，仅支持Linux和macOS。
