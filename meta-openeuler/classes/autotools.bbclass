...
autotools_do_configure() {
	...
	if [ -e ${AUTOTOOLS_SCRIPT_PATH}/configure.in -o -e ${AUTOTOOLS_SCRIPT_PATH}/configure.ac ]; then
		...
		automake --version
		echo "AUTOV is $AUTOV"
		if [ -d ${NATIVESDK_DATADIR}/aclocal-$AUTOV ]; then
			ACLOCAL="$ACLOCAL --automake-acdir=${NATIVESDK_DATADIR}/aclocal-$AUTOV"
		fi
		...
		if grep -q "^[[:space:]]*AM_GLIB_GNU_GETTEXT" $CONFIGURE_AC; then
			...
			cp ${NATIVESDK_DATADIR}/gettext/config.rpath ${AUTOTOOLS_AUXDIR}/
			if [ -d ${S}/po/ ]; then
				cp -f ${NATIVESDK_DATADIR}/gettext/po/Makefile.in.in ${S}/po/
				if [ ! -e ${S}/po/remove-potcdate.sin ]; then
					cp ${NATIVESDK_DATADIR}/gettext/po/remove-potcdate.sin ${S}/po/
				fi
			fi
