# openeuler's meson.bbclass is almost the same as poky's meson.bbclass. There are two modification points: 
...
# this is a workaround, meson.native: pkgconfig = 'pkg-config-native' -> pkgconfig = 'pkg-config'
do_write_config() {
...
EOF
    ...
    # use pkg-config command instead of pkg-config-native command
