...
CFGDIR="${ROOT_DIR}/etc/default/openeuler-volatiles"
...
create_file() {
	...
	test "$VOLATILE_ENABLE_CACHE" = yes && echo "$EXEC" >> /etc/openeuler-volatile.cache.build
...
check_requirements() {
	...
	TMP_INTERMED="${TMPROOT}/openeuler-tmp.$$"
	TMP_DEFINED="${TMPROOT}/openeuler-tmpdefined.$$"
	TMP_COMBINED="${TMPROOT}/openeuler-tmpcombined.$$"
	...
	mount --bind \"${TSOURCE}\" \"${TNAME}\""
		...
		case "${TTYPE}" in
			...
			"d")  [ "${VERBOSE}" != "no" ] && echo "Creating directory -${TNAME}-."
				...
				;;
			"p")  [ "${VERBOSE}" != "no" ] && echo "Changing permission -${TNAME}-."
				ch_perm "${TNAME}"
				;;
...
if test -e "${ROOT_DIR}/etc/openeuler-volatile.cache" -a "$VOLATILE_ENABLE_CACHE" = "yes" -a "x$1" != "xupdate" -a "x$clearcache" = "x0"
then
	sh "${ROOT_DIR}/etc/openeuler-volatile.cache"
else
	rm -f "${ROOT_DIR}/etc/openeuler-volatile.cache" "${ROOT_DIR}/etc/openeuler-volatile.cache.build"
	...
	TMP_FILE="${TMPROOT}/openeuler-tmp_volatile.$$"
	rm -f "$TMP_FILE"
    touch ${CFGDIR}/endtest
	...
	[ -e "${ROOT_DIR}/etc/openeuler-volatile.cache.build" ] && sync && mv "${ROOT_DIR}/etc/openeuler-volatile.cache.build" "${ROOT_DIR}/etc/openeuler-volatile.cache"
fi
