...
sed -i 's| init=/usr/lib/init_resize\.sh||' /boot/cmdline.txt
[ -f "/boot/EFI/BOOT/grub.cfg" ] && sed -i 's| init=/usr/lib/init_resize\.sh||' /boot/EFI/BOOT/grub.cfg
...
if main; then
  echo "Resized root filesystem. Rebooting in 5 seconds..." > /dev/kmsg
  sleep 5
else
  echo "Could not expand filesystem: ${FAIL_REASON}" > /dev/kmsg
  sleep 5
fi
